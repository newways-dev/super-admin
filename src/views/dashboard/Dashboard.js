...
const Dashboard = () => {
  ...
  const createEmployee = async () => {
    await addDoc(usersCollection, { name: name, role: role })
  }
    ...
    <>
      {users.map(user => (
        <div key={user.id}>
          ...
          <button onClick={() => changeRole(user.id)} style={{ marginRight: '5px' }}>Change Role</button>
          <button onClick={() => deteteEmployee(user.id)}>Delete Employee</button>
          <hr></hr>
        </div>
      ))}
